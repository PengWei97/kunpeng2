...
/**
 * GrGrFiniteStrainCrystalPlasticity uses the multiplicative decomposition of deformation gradient
 * and solves the PK2 stress residual equation at the intermediate configuration to evolve the
 * material state.
 * The internal variables are updated using an interative predictor-corrector algorithm.
 * Backward Euler integration rule is used for the rate equations.
 */
class GrGrFiniteStrainCrystalPlasticity : public ComputeStressBase
{
public:
  static InputParameters validParams();

  GrGrFiniteStrainCrystalPlasticity(const InputParameters & parameters);
...
protected:
  ...
  MaterialProperty<RankTwoTensor> & _pk2_grgr;
  const MaterialProperty<RankTwoTensor> & _pk2_old;
  MaterialProperty<RankTwoTensor> & _lag_e;
  MaterialProperty<RankTwoTensor> & _lag_e_grgr;
