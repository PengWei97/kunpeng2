#include "ComputerGrGrLine2ElasticEnergy.h"
#include "RankTwoTensor.h"
#include "RankFourTensor.h"

registerMooseObject("PhaseFieldApp", ComputerGrGrLine2ElasticEnergy);

InputParameters
ComputerGrGrLine2ElasticEnergy::validParams()
{
...
ComputerGrGrLine2ElasticEnergy::ComputerGrGrLine2ElasticEnergy(const InputParameters & parameters)
...
void
ComputerGrGrLine2ElasticEnergy::initialSetup()
{
  validateCoupling<RankTwoTensor>(_base_name + "elastic_strain");
}

Real
ComputerGrGrLine2ElasticEnergy::computeF()
{
  ...
  for (MooseIndex(op_to_grains) op_index = 0; op_index < op_to_grains.size(); ++op_index)
  {
    ...
    C_deriv = (_grain_tracker.getData(grain_id) - _stiff_tensor[_qp]) * dhdopi / sum_h;
    ...
    EE_deriv = 0.5 * (C_deriv * _elastic_strain[_qp]).doubleContraction((_elastic_strain[_qp]));
