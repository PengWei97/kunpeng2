...
#include "GrGrFiniteStrainCrystalPlasticity.h"
...
registerMooseObject("kunpengApp", GrGrFiniteStrainCrystalPlasticity);

InputParameters
GrGrFiniteStrainCrystalPlasticity::validParams()
{
...
GrGrFiniteStrainCrystalPlasticity::GrGrFiniteStrainCrystalPlasticity(const InputParameters & parameters)
  : ComputeStressBase(parameters),
    ...
    _pk2_grgr(declareProperty<RankTwoTensor>("pk2_grgr")), // 2nd Piola-Kirchoff Stress
    ...
    _lag_e_grgr(declareProperty<RankTwoTensor>("lage_grgr")), // Lagrangian strain
    _lag_e_old(
...
void
GrGrFiniteStrainCrystalPlasticity::initQpStatefulProperties()
{
  _stress[_qp].zero();
...
void
GrGrFiniteStrainCrystalPlasticity::initSlipSysProps()
{
...
void
GrGrFiniteStrainCrystalPlasticity::assignSlipSysRes()
{
...
void
GrGrFiniteStrainCrystalPlasticity::readFileInitSlipSysRes()
{
  _gss[_qp].resize(_nss);
  ...
  for (unsigned int i = 0; i < _nss; ++i)
    if (!(file >> _gss[_qp][i]))
      mooseError("Error GrGrFiniteStrainCrystalPlasticity: Premature end of slip_sys_res_prop file");
...
void
GrGrFiniteStrainCrystalPlasticity::getInitSlipSysRes()
{
  if (_gprops.size() <= 0)
    mooseError("GrGrFiniteStrainCrystalPlasticity: Error in reading slip system resistance properties: "
  ...
  for (unsigned int i = 0; i < _gprops.size() / num_data_grp; ++i)
  {
    ...
    if (vs <= 0 || ve <= 0)
      mooseError("GrGrFiniteStrainCrystalPlasticity: Indices in gss property read must be positive "
                 "integers: is = ",
                 vs,
                 " ie = ",
                 ve);
    ...
    if (vs != floor(vs) || ve != floor(ve))
      mooseError("GrGrFiniteStrainCrystalPlasticity: Error in reading slip system resistances: Values "
    ...
    if (is > ie)
      mooseError("GrGrFiniteStrainCrystalPlasticity: Start index is = ",
                 is,
  ...
  for (unsigned int i = 0; i < _nss; ++i)
    if (_gss[_qp][i] <= 0.0)
      mooseError("GrGrFiniteStrainCrystalPlasticity: Value of resistance for slip system ",
                 i + 1,
                 " non positive");
}
...
void
GrGrFiniteStrainCrystalPlasticity::readFileFlowRateParams()
{
  _a0.resize(_nss);
  _xm.resize(_nss);
  ...
  for (unsigned int i = 0; i < _nss; ++i)
  {
    for (unsigned int j = 0; j < _num_slip_sys_flowrate_props; ++j)
      if (!(file >> vec[j]))
        mooseError(
            "Error GrGrFiniteStrainCrystalPlasticity: Premature end of slip_sys_flow_rate_param file");
...
void
GrGrFiniteStrainCrystalPlasticity::getFlowRateParams()
{
  if (_flowprops.size() <= 0)
    mooseError("GrGrFiniteStrainCrystalPlasticity: Error in reading flow rate  properties: Specify "
  ...
  for (unsigned int i = 0; i < _flowprops.size() / num_data_grp; ++i)
  {
    ...
    if (vs <= 0 || ve <= 0)
      mooseError("GrGrFiniteStrainCrystalPlasticity: Indices in flow rate parameter read must be "
    ...
    if (vs != floor(vs) || ve != floor(ve))
      mooseError("GrGrFiniteStrainCrystalPlasticity: Error in reading flow props: Values specifying "
  ...
  for (unsigned int i = 0; i < _nss; ++i)
  {
    ...
    {
      mooseWarning(
          "GrGrFiniteStrainCrystalPlasticity: Non-positive flow rate parameters ", _a0(i), ",", _xm(i));
      break;
    }
  }
}
...
void
GrGrFiniteStrainCrystalPlasticity::readFileHardnessParams()
{
}

// Read hardness parameters from .i file
void
GrGrFiniteStrainCrystalPlasticity::getHardnessParams()
{
  if (_hprops.size() <= 0)
    mooseError("GrGrFiniteStrainCrystalPlasticity: Error in reading hardness properties: Specify input "
...
void
GrGrFiniteStrainCrystalPlasticity::getSlipSystems()
{
...
void
GrGrFiniteStrainCrystalPlasticity::initAdditionalProps()
{
}
 ...
 */
void
GrGrFiniteStrainCrystalPlasticity::computeQpStress()
{
  ...
  while (_err_tol && _max_substep_iter > 1)
  {
    ...
    if (substep_iter > _max_substep_iter)
      mooseWarning("GrGrFiniteStrainCrystalPlasticity: Failure with substepping");
#endif
...
void
GrGrFiniteStrainCrystalPlasticity::preSolveQp()
{
  // Initialize variable
  if (_first_substep)
  {
...
void
GrGrFiniteStrainCrystalPlasticity::solveQp()
{
  preSolveStatevar();
  solveStatevar();
  if (_err_tol)
...
void
GrGrFiniteStrainCrystalPlasticity::postSolveQp()
{
  if (_err_tol)
  {
    _err_tol = false;
    if (_gen_rndm_stress_flag)
    {
      ...
      mooseError("GrGrFiniteStrainCrystalPlasticity: Constitutive failure");
  }
  else
  {
    _pk2_grgr[_qp] = _pk2[_qp];
...
void
GrGrFiniteStrainCrystalPlasticity::preSolveStatevar()
{
...
void
GrGrFiniteStrainCrystalPlasticity::solveStatevar()
{
  Real gmax, gdiff;
  unsigned int iterg;
  ...
  {
#ifdef DEBUG
    mooseWarning("GrGrFiniteStrainCrystalPlasticity: Hardness Integration error gmax", gmax, "\n");
#endif
    _err_tol = true;
  }
}
...
void
GrGrFiniteStrainCrystalPlasticity::postSolveStatevar()
{
  if (_max_substep_iter == 1) // No substepping
  {
    _gss[_qp] = _gss_tmp;
...
void
GrGrFiniteStrainCrystalPlasticity::preSolveStress()
{
  if (_max_substep_iter == 1) // No substepping
  {
...
void
GrGrFiniteStrainCrystalPlasticity::solveStress()
{
  unsigned int iter = 0;
...
#ifdef DEBUG
    mooseWarning(
        "GrGrFiniteStrainCrystalPlasticity: Slip increment exceeds tolerance - Element number ",
        _current_elem->id(),
        " Gauss point = ",
        _qp);
#endif
         ...
         iter < _maxiter) // Check for stress residual tolerance
  {
    ...
    {
#ifdef DEBUG
      mooseWarning("GrGrFiniteStrainCrystalPlasticity: Failed with line search");
#endif
      _err_tol = true;
      return;
    }
  ...
  {
#ifdef DEBUG
    mooseWarning("GrGrFiniteStrainCrystalPlasticity: Stress Integration error rmax = ", rnorm);
#endif
    _err_tol = true;
  }
}
...
void
GrGrFiniteStrainCrystalPlasticity::postSolveStress()
{
  if (_max_substep_iter == 1) // No substepping
  {
...
void
GrGrFiniteStrainCrystalPlasticity::update_slip_system_resistance()
{
  updateGss();
}
 ...
 */
void
GrGrFiniteStrainCrystalPlasticity::updateGss()
{
...
void
GrGrFiniteStrainCrystalPlasticity::calc_resid_jacob(RankTwoTensor & resid, RankFourTensor & jac)
{
  calcResidual(resid);
  if (_err_tol)
    return;
...
void
GrGrFiniteStrainCrystalPlasticity::calcResidual(RankTwoTensor & resid)
{
...
void
GrGrFiniteStrainCrystalPlasticity::calcJacobian(RankFourTensor & jac)
{
...
void
GrGrFiniteStrainCrystalPlasticity::getSlipIncrements()
{
  for (unsigned int i = 0; i < _nss; ++i)
  {
...
RankTwoTensor
GrGrFiniteStrainCrystalPlasticity::get_current_rotation(const RankTwoTensor & a)
{
  return getMatRot(a);
}
...
RankTwoTensor
GrGrFiniteStrainCrystalPlasticity::getMatRot(const RankTwoTensor & a)
{
  RankTwoTensor rot;
  ...
  if (info != 0)
    mooseError("GrGrFiniteStrainCrystalPlasticity: DSYEV function call in getMatRot function failed");
...
void
GrGrFiniteStrainCrystalPlasticity::computeQpElasticityTensor()
{
}

RankFourTensor
GrGrFiniteStrainCrystalPlasticity::calcTangentModuli()
{
...
void
GrGrFiniteStrainCrystalPlasticity::calc_schmid_tensor()
{
...
RankFourTensor
GrGrFiniteStrainCrystalPlasticity::elastoPlasticTangentModuli()
{
  RankFourTensor tan_mod;
...
RankFourTensor
GrGrFiniteStrainCrystalPlasticity::elasticTangentModuli()
{
  return _elasticity_tensor[_qp]; // update jacobian_mult
}

bool
GrGrFiniteStrainCrystalPlasticity::line_search_update(const Real rnorm_prev, const RankTwoTensor dpk2)
{
...
void
GrGrFiniteStrainCrystalPlasticity::internalVariableUpdateNRiteration()
{
